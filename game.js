...
Game.prototype.getNeighbors = function(rowIndex, cellIndex) {
    var rowIndexMax = this.board.length - 1,
        colIndexMax = this.board[0].length - 1,
        directions = [-1, 0, 1],
        neighbors = [];
    ...
    directions.forEach(function(rowDirection) {
        directions.forEach(function(columnDirection) {
            if (!(rowDirection === 0 && columnDirection === 0)) {
                ...
                // wrapping
                if (neighbor[0] === -1) {
                    neighbor[0] = rowIndexMax;
                }
                if (neighbor[0] === rowIndexMax + 1) {
                    neighbor[0] = 0;
                }
                if (neighbor[1] === -1) {
                    neighbor[1] = colIndexMax;
                }
                if (neighbor[1] === colIndexMax + 1) {
                    neighbor[1] = 0;
                }
